...
public class OLB_Controller {
	...
	public OLBUserInfo getUserInfo(@PathVariable ("username")String userid)
	{
		if(userid!=null && userid!="")
		{
			...
			olbuserinfo.setUsername(userid);
			olbuserinfo.setAccountNo(account.getSavingsAccountNumber()+"");
			olbuserinfo.setBalance(account.getSavingsAvailableBalance()+"");
			olbuserinfo.setTransactions(getTransactions(account.getSavingsAccountNumber()+""));
			System.out.println("User Information requested by OAuth Resource Server ---> "+olbuserinfo.toString());
			return olbuserinfo;
		}
		else
			return null;
